...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    setLoading(true);
    const result = await login(email, password, requiresOtp ? otp : null, ['sales']);
    if (result.requiresOtp) {
      ...
      if (result.user?.role === 'sales') {
        ...
        logout();
        setError('Access denied. Sales portal requires sales privileges.');
        setLoading(false);
        return;
      }
    } else {
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                ...
                <div className="lp-input-wrap" style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    placeholder="sales@megapacific.com"
                    required
                    autoComplete="email"
                    style={{
                      width: '100%',
              ...
              >
                {loading ? (
                  ...
                  requiresOtp ? 'Verify OTP & Login' : 'Login Sales'
                )}
              </button>
            </form>
          </div>
