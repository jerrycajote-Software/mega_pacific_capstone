          ...
          <>
            ...
            <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 3, bgcolor: '#f9fafb', display: 'flex', flexDirection: 'column', gap: 2 }}>
              {selectedInquiry.messages.map((msg, index) => {
                const isSales = msg.sender.role === 'sales' || msg.sender.role === 'admin';
                return (
                  <Box key={index} sx={{ alignSelf: isSales ? 'flex-end' : 'flex-start', maxWidth: '70%' }}>
                    {!isSales && (
                    ...
                    <Box sx={{
                      bgcolor: isSales ? 'primary.main' : 'white',
                      color: isSales ? 'white' : 'text.primary',
                      p: 2,
                      borderRadius: 3,
                      boxShadow: '0 2px 8px rgba(0,0,0,0.05)',
                      borderTopLeftRadius: !isSales ? 0 : undefined,
                      borderTopRightRadius: isSales ? 0 : undefined,
                    }}>
                      <Typography variant="body1">{msg.content}</Typography>
                    </Box>
                    <Typography variant="caption" sx={{ mt: 0.5, display: 'block', textAlign: isSales ? 'right' : 'left', color: 'text.disabled' }}>
