...
const StatCard = ({ label, value, icon, iconBg }) => (
  <div className="widget" style={{ padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1.25rem', position: 'relative', overflow: 'hidden' }}>
    <div style={{ padding: '0.85rem', borderRadius: 14, background: iconBg, border: `1px solid ${iconBg.replace('0.08', '0.15')}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {icon}
    </div>
    <div>
      <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</div>
      <div style={{ fontSize: '1.875rem', fontWeight: 800, color: 'var(--text-primary)', marginTop: 4 }}>{value}</div>
    </div>
  </div>
);
...
const SalesDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/api/sales/dashboard`, {
  ...
  const { counts, inventorySummary, lowStockProducts, recentlyAdded, recentOrders, dailyActivity, totals, topProducts } = data;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--green)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Sales Dashboard</span>
          </div>
      ...
      {/* Operational Counts Cards (Moved slightly down, replaced top with Sales Overview) */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem' }}>
        <StatCard label="Total Revenue" value={totals?.revenue || '₱0'} icon={<TrendingUpIcon sx={{ fontSize: 28, color: '#22c55e' }} />} iconBg="rgba(34,197,94,0.08)" />
        <StatCard label="Total Products" value={totals?.products || 0} icon={<Inventory2Icon sx={{ fontSize: 28, color: '#60a5fa' }} />} iconBg="rgba(96,165,250,0.08)" />
        <StatCard label="Total Orders" value={totals?.orders || 0} icon={<ShoppingCartIcon sx={{ fontSize: 28, color: '#c084fc' }} />} iconBg="rgba(192,132,252,0.08)" />
      </div>
      ...
      <div className="widget" style={{ padding: '1.25rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h3 style={{ margin: 0, fontSize: '0.9375rem', fontWeight: 700, color: 'var(--text-primary)' }}>Top Selling Products</h3>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
          ...
          <tbody>
            {!topProducts || topProducts.length === 0 ? (
              <tr>
                <td colSpan="4" style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>No sales data yet.</td>
              </tr>
            ) : (
              topProducts.map((p, i) => (
                <tr key={i} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.015)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                  <td style={{ ...td(), fontWeight: 500, color: 'var(--text-primary)' }}>{p.name}</td>
                  <td style={{ ...td(), color: 'var(--text-muted)' }}>{p.type}</td>
                  <td style={{ ...td(), textAlign: 'center', fontWeight: 600 }}>{p.sold}</td>
                  <td style={{ ...td(true), fontWeight: 600, color: 'var(--green)' }}>{p.revenue}</td>
                </tr>
              ))
            )}
          </tbody>
