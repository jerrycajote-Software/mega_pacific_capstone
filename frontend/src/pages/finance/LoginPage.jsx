...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    setLoading(true);
    const result = await login(email, password, requiresOtp ? otp : null, ['finance']);
    if (result.requiresOtp) {
      ...
      if (result.user?.role === 'finance') {
        ...
        logout();
        setError('Access denied. Finance portal requires finance privileges.');
        setLoading(false);
        return;
      }
    } else {
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                ...
                <div className="lp-input-wrap" style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    placeholder="finance@megapacific.com"
                    required
                    autoComplete="email"
                    style={{
                      width: '100%',
              ...
              >
                {loading ? (
                  ...
                  requiresOtp ? 'Verify OTP & Login' : 'Login Finance Staff'
                )}
              </button>
            </form>
          </div>
