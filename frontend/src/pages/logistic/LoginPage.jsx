...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    setLoading(true);
    const result = await login(email, password, requiresOtp ? otp : null, ['logistic']);
    if (result.requiresOtp) {
      setRequiresOtp(true);
      setMsg(result.message || 'OTP sent to your email.');
    } else if (result.success) {
      if (result.user?.role === 'logistic') {
        navigate('/logistic/dashboard', { replace: true });
      } else {
        logout();
        setError('Access denied. Logistic portal requires logistic privileges.');
        setLoading(false);
        return;
      }
    } else {
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                ...
                <div className="lp-input-wrap" style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    placeholder="logistic@megapacific.com"
                    required
                    autoComplete="email"
                    style={{
                      width: '100%',
              ...
              >
                {loading ? (
                  ...
                  requiresOtp ? 'Verify OTP & Login' : 'Login Logistic Staff'
                )}
              </button>
            </form>
          </div>
