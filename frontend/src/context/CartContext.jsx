...
export const CartProvider = ({ children }) => {
  const [cartItems, setCartItems] = useState(() => {
    try {
      const saved = localStorage.getItem('mega_pacific_cart');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('mega_pacific_cart', JSON.stringify(cartItems));
    } catch (e) {
      ...
      const validatedItems = await Promise.all(cartItems.map(async (item) => {
        try {
          ...
          if (res.data.success) {
            ...
            let latestPrice = item.price; // Default to existing
            ...
            if (item.variant) {
              ...
              if (variant) {
                latestPrice = variant.price; // Sync real-time variant price
                item.variant = variant; // Sync real-time variant stock
              }
            } else {
              if (product.stock === 0) {
                isOutOfStock = true;
              }
              latestPrice = product.price; // Sync real-time base price
            }
            item.product = product; // Sync real-time product stock
          ...
          return item; // Keep as is on network error
        }
      }));
  ...
  const addToCart = (product, variant, quantity) => {
    setCartItems(prev => {
      // Determine unique ID based on product and variant
